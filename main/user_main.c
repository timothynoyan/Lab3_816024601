...
void app_main(void){	
	...
	/* UNIT TEST: LED Off Function */
	/* Stub for turning LED On */
	gpio_set_level(GPIO_OUTPUT, 1);
	/* Task-delay for 1 second */
	vTaskDelay(1000 / portTICK_RATE_MS);
	/* Task to turn LED Off */
	xTaskCreate(task2, "task2", 2048, NULL, 3, &task_handle);
	/* Check if pin is high and use task handler to stop running task 1 */
	if (gpio_get_level(GPIO_OUTPUT) == 0 && task_handle != NULL){
		ESP_LOGI(TAG, "UNIT TEST: task2 was successful\n");
